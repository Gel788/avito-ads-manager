...
class Config:
    ...
    MAX_CONTENT_LENGTH = int(os.getenv('MAX_CONTENT_LENGTH', str(16 * 1024 * 1024)))  # 16MB по умолчанию
    ...
    LOG_FILE = os.path.join('logs', os.getenv('LOG_FILE', 'avito_ads.log'))
    if not os.path.exists('logs'):
        os.makedirs('logs', mode=0o777, exist_ok=True)
    
    # Настройки XML файла
    XML_TEMPLATE_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'templates')
    if not os.path.exists(XML_TEMPLATE_DIR):
        os.makedirs(XML_TEMPLATE_DIR, mode=0o777, exist_ok=True)
    XML_TEMPLATE_PATH = os.getenv('XML_TEMPLATE_PATH', os.path.join(XML_TEMPLATE_DIR, 'avito.xml'))
    
    # Ограничения файлов
    MAX_FILE_SIZE = int(os.getenv('MAX_FILE_SIZE', str(50 * 1024 * 1024)))  # 50 MB
    MAX_PHOTO_SIZE = int(os.getenv('MAX_PHOTO_SIZE', str(10 * 1024 * 1024)))  # 10 MB
    
    # Ad settings
    MAX_PHOTOS_PER_AD = int(os.getenv('MAX_PHOTOS_PER_AD', '30'))
    ...
    MIN_REPOST_INTERVAL = MIN_REPOST_INTERVAL 